...
ChunkColumn::ChunkColumn() : position(0), highest_natural_point(-1), mesh(), blockStore(), isFlat(1), fromFile(0), needsSave(0), stage(200), buffer()
{
	// blocks.fill(Blocks::AIR);
}

ChunkColumn::ChunkColumn(glm::vec2 pos) : position(pos), highest_natural_point(-1), mesh(), isFlat(0), stage(0), fromFile(0), needsSave(0), buffer()
{
	blockStore = new BlockStore(pos);
}

ChunkColumn::ChunkColumn(std::string fileName) : fromFile(1), needsSave(0), buffer()
{
...
ChunkColumn::ChunkColumn(glm::vec2 pos, WorldMap* worldMap) : position(pos), highest_natural_point(-1), mesh(), isFlat(0), stage(0), fromFile(0), needsSave(0), buffer() {
